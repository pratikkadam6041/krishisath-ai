...
#else
const int AIR_VALUE = 586;
const int WATER_VALUE = 330;
const char* ZONE_ID = "z2";
...
void printConfiguredWifiScan() {
  ...
  for (int i = 0; i < networkCount; i++) {
    ...
    if (WiFi.SSID(i) == WIFI_SSID) {
      found = true;
      Serial.print("[Wi-Fi] Found SSID. RSSI=");
      Serial.print(WiFi.RSSI(i));
      Serial.print(" dBm | Channel=");
      Serial.print(WiFi.channel(i));
      Serial.print(" | Encryption=");
  ...
  if (!found) {
    Serial.println("[Wi-Fi] Configured SSID not found. Copy the exact 2.4GHz SSID spelling from the list above.");
  }
...
bool connectWiFi(bool force = false) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    Serial.println(")");
    Serial.println("[Wi-Fi] Check SSID/password, use 2.4GHz, and avoid captive-login public Wi-Fi.");
